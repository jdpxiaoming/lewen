...
public class Activity_Consume_Record extends Activity {
...
//					Packet.close();
	...
	public void init() {
		...
		listview	=	(ListView) findViewById(R.id.listviewOfCosumeRecord);
		progressar	=	(LinearLayout) findViewById(R.id.progressbarOfConsumeRecord);
		...
		if(MyApplication.rsp_login!=null){
			...
			MyApplication.packet.getConsumeRecord(MyApplication.rsp_login.getUserId(), handler);
		}else{
			MyApplication.getInstance().throwTips("您当前没有消费记录，请先登录！");
			startActivity(new Intent(Activity_Consume_Record.this,Activity_Login.class));
			finish();
		}
	}
