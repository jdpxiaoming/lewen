...
public class ChatActivity extends Activity implements OnClickListener {
	...
	private TextView mBtnRcd, textTitle;
	...
	private AudioEncoder encoder;//编码的类
...
//			}
	...
	public void initView() {
		...
		// set title
		textTitle = (TextView) findViewById(R.id.textTitleOfChat);
		String toUserId = getIntent().getStringExtra("toUserId");
		textTitle.setText(toUserId);
		...
		if (btn_vocie) {
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN && flag == 1) {
				...
				if (event.getY() >= del_Y
					...
					if(null==encoder){
						encoder = new AudioEncoder(toUserId, mHandler);
					}
					encoder.startEncoder(android.os.Environment.getExternalStorageDirectory()+ "/my/"
							+ voiceName);//开始编码
				}
			}
